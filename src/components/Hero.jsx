...
const Home = () => {
  // --- ANTIGRAVITY / MOUSE PARALLAX SETUP ---
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);

  // Smooth out the mouse movement for that "floating" feel
  const smoothX = useSpring(mouseX, { stiffness: 40, damping: 30 });
  const smoothY = useSpring(mouseY, { stiffness: 40, damping: 30 });

  // Calculate movements for different layers (Parallax Effect)
  // Layer 1 moves WITH the mouse slightly
  const x1 = useTransform(smoothX, [0, window.innerWidth], [-20, 20]);
  const y1 = useTransform(smoothY, [0, window.innerHeight], [-20, 20]);

  // Layer 2 moves AGAINST the mouse (Background depth)
  const x2 = useTransform(smoothX, [0, window.innerWidth], [50, -50]);
  const y2 = useTransform(smoothY, [0, window.innerHeight], [50, -50]);

  // Layer 3 moves aggressively (Foreground/Floating elements)
  const x3 = useTransform(smoothX, [0, window.innerWidth], [-100, 100]);
  const y3 = useTransform(smoothY, [0, window.innerHeight], [-100, 100]);
  ...
  return (
    <div className="overflow-hidden bg-slate-50 dark:bg-slate-950 transition-colors duration-300 font-sans">
      ...
      {/* 1. HERO SECTION (With Antigravity Effect) */}
      <section className="relative bg-slate-900 pt-32 pb-20 lg:pt-40 lg:pb-32 overflow-hidden min-h-[90vh] flex items-center">
  
        {/* --- Antigravity Background Effects --- */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          
          {/* Layer 1: Grid (Subtle Movement) */}
          <motion.div 
            style={{ x: x1, y: y1 }}
            className="absolute inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:24px_24px]"
          />

          {/* Layer 2: Glowing Orbs (Opposite Movement) */}
          <motion.div 
            style={{ x: x2, y: y2 }}
            className="absolute top-[-10%] right-[-5%] w-[500px] h-[500px] bg-indigo-600/20 rounded-full blur-[100px] opacity-60"
          />
          <motion.div 
            style={{ x: x3, y: y3 }} // Faster movement
            className="absolute bottom-[-10%] left-[-10%] w-[400px] h-[400px] bg-emerald-500/10 rounded-full blur-[100px] opacity-60"
          />

          {/* Layer 3: Floating Particles (Antigravity feel) */}
          <motion.div style={{ x: x2, y: x3 }} className="absolute top-1/4 left-1/4 w-2 h-2 bg-indigo-400 rounded-full blur-[1px] opacity-50" />
          <motion.div style={{ x: x3, y: y2 }} className="absolute bottom-1/3 right-1/4 w-3 h-3 bg-emerald-400 rounded-full blur-[2px] opacity-40" />
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">
            ...
            {/* RIGHT SIDE: Visual/Code Mockup (Applying 3D Float) */}
            <motion.div 
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.8 }}
              style={{ x: x1, y: y1 }} // Make the code block float slightly
              className="relative hidden lg:block perspective-1000"
            >
              {/* Background Glow for Code Block */}
              <motion.div style={{ x: x2, y: y2 }} className="absolute inset-0 bg-gradient-to-tr from-indigo-500 to-emerald-500 blur-[80px] opacity-20 -z-10"></motion.div>
              ...
              <div className="relative bg-slate-950 rounded-xl border border-slate-800 shadow-2xl p-6 transform rotate-1 hover:rotate-0 transition-transform duration-500">
                ...
                {/* Floating Badge 1 */}
                <motion.div 
                  style={{ x: x3, y: y3 }} // Moves faster for depth
                  className="absolute -bottom-6 -right-6 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 flex items-center gap-3"
                >
                ...
                <motion.div 
                  style={{ x: y2, y: x2 }} 
                  className="absolute top-1/2 right-8 bg-indigo-600 p-3 rounded-full shadow-lg shadow-indigo-500/50 text-white"
                >
